...
/*
 * Updates by Thomas Dean, Jan 2021
 *
 *   - Added new flag -n which specifies the name of the table that
 *	has the bytecodes.
 *   - fixes for compiling on Mac/Linux
 *	- main returns int
 *	- added include files
 *	- removed extern for malloc (in include file now)
 *   - also styliesticly changed enums to singular from plural.
 */
...
typedef enum {
	...
	fChoiceRuleFailed			/* 1 */
} FailureCode;			/* S/SL System Failure Code Type */
...
ErrorCode firstFatalErrorCode = eSslStackOverflow;
...
/* Special token returned by Input Scanner for garbage */
	tIllegal				/* 30 */
} InputToken;			/* Input Token Type */
#endif	/* SELFCONTAINED */

InputToken firstCompoundToken = tIdent;
InputToken lastCompoundToken = tInteger;
InputToken firstInputToken = tSyntaxError;
InputToken lastInputToken = tIllegal;
...
ErrorCode firstUserErrorSignalValue = eCycleHasNoExits;
 ...
 */
InputToken compoundToken;	/* Last compound input token accepted */
...
InputToken keywordToken[] = { tMechanism, tOutput, tInput, tError, tRules,
	...
	char *argv[];
{
	...
	while (processing) {
		...
		case oGenerateDefinitions:
			...
			break;
		case oGenerateTable:
			GenerateOutputTable(typeName, tableName);
...
void GenerateSymbolDefinitions() {
	SymbolIndex s;
	SymbolClass c;
...
void GenerateOutputTable(char * typeName, char *tableName) {
	...
	fprintf(tablefp, "/* S/SL table (%d entries) */\n",
			    outputPointer);
	fprintf(tablefp, "\n%s %s[] = {\n\t", typeName ? typeName:"int", tableName?tableName:"sslTable");
...
void SslFailure(FailureCode failCode) {
