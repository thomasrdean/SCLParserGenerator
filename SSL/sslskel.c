...
STATIC ErrorCode	firstFatalErrorCode = eSslStackOverflow;  /* fix */
...
/* Input Interface */
STATIC FILE	*infp;
STATIC InputToken	nextInputToken = tNewLine;
STATIC InputToken	savedToken;
...
STATIC InputToken	compoundToken;	/* Last compound input token accepted */
...
STATIC InputToken	savedInputToken;


/* This procedure emits the error message associated with errCode */

STATIC void Error(ErrorCode errCode){
...
STATIC void EmitOutputToken(OutputToken	emittedToken) {
...
STATIC void SslGenerateCompoundInputToken(InputToken expectedToken) {
...
STATIC void SslChoice(int choiceTag) {
