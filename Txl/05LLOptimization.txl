...
%   - no lookahead block generated.
% Example 2
% A @ LL 0 1 1 | B @ LL 0 1 2 | C | D @ LL 0 1 3 | E @ LL 0 1 3
%  - C had no lookahead, So also not optimizable.
%   - no lookahead block generated.
% 
% for rest see docs...
%
...
function buildLLKBlock Short [id] TypeDec2 [type_decision]
    ...
    % otherwise no optimization to be done.
    % may want to remove this choice to to simplify the code generator
    ...
    construct TypeRefs [repeat type_reference]
	_ [^  TypeDec2]
    construct LABlock [refs_or_sub_block]
    	TypeRefs

    construct LBlock [optimizable_block]
	<lookahead>
	    LABlock [buildNestedBlock Short]
	</lookahead>

    % did it change?
    % not sure if this should be here
    % may want to use the exact same code for imitting all lists of
    % types, in this case the list should be in the lookahead blockj
    % not as alternates (i.e. |)
    %deconstruct LBlock
	%<lookahead>
	   %LABlock [lookahead_block]
	%</lookahead>
...
function buildNestedBlock Short [id]

    replace [refs_or_sub_block]
	TypeRefs [repeat type_reference]

    % first find the common minimum offset that applies to 
    % all type references. LL annotations are sorted by offset
    % if a given offset is not common to all branches, or has different
    % sizes, then move to the second list
    construct MinTypeRefs [repeat type_reference]
    	TypeRefs [findCommonMinOffset Short]
	         %[message "After findCommonMinOffset"]
		 %[print]

    % fail to make a change if no common offset
    deconstruct MinTypeRefs
	_ [id] _ [opt dotID] UsableLL [llk_annotation] _ [repeat annotation_item] _ [opt unused_annotation]
	Rest [repeat type_reference]
    
    % at this point in time, all of the first LLK annotations in the list will be
    % the same offset and same size
    deconstruct UsableLL
       @ LL MinOffset [number] SizeOfMin [number] MinVals [list number]

    construct Cases [repeat switch_case]
       _ [addCase MinOffset each MinTypeRefs]
         [sortCasesByValue]
...
% this is supposed to find the lowest common offset of all
% of the choices.
%
% Note those new to TXL - The rule..skipping with the same repeat type
% is used to repeatedly apply the rule at the root of the repeat
% until the rule fails, without applying the rule inside of the repeat

rule findCommonMinOffset Short [id]
    skipping [repeat type_reference]
    replace [repeat type_reference]
        TypeRefs [repeat type_reference]

    % Findthe minimum offset in the typerefs.
    % start by finding the first offset in the references
    deconstruct TypeRefs
	_ [id] _ [opt dotID] UsableLL [llk_annotation] _ [repeat annotation_item] _ [opt unused_annotation]
	Rest [repeat type_reference]
    deconstruct UsableLL
    ...
    % this rule applies if all of the first annotations
    % are the same offset and same size
    where not all
    	SizeOfMin [checkSizeAndOffset MinOffset SizeOfMin each TypeRefs]

    %construct Msg3 [stringlit]
    	%_ [+ '"Not all min offsets for type decision "]
	  %[+ Short]
	  %[+ '" are of size "]
	  %[+ SizeOfMin]
	  %[print]
    ...
    construct TypeRefsWithoutSize [repeat type_reference]
	_ [addIfSizeIsNot Size each TypeRefs]
	  %[putp "Typrefs without size are %"]
    by
    	TypeRefs [moveAnnotationToUnused MinOffset]
end rule
...
function checkSizeAndOffset MinOffset [number] MinSize [number] aTypeRef [type_reference]
    deconstruct * [llk_annotation] aTypeRef
    	'@ LL Offset [number]  Size [number] Vals [list number]
    deconstruct Offset
        MinOffset
...
function addIfOffsetIsNot MinOffset [number] aTypeRef [type_reference]
    % two level deconstruct to force first mactch
    deconstruct * [llk_annotation] aTypeRef
    	'@ LL Offset[number] _ [number] Values [list number]
    deconstruct not Offset
    	MinOffset
    replace [repeat type_reference]
...
function addIfSizeIsNot MinSize [number] aTypeRef [type_reference]
    deconstruct * [llk_annotation] aTypeRef
    	'@ LL Offset[number] Size [number] Values [list number]
...
rule moveAnnotationToUnused MinOffset [number]
    replace $ [type_reference]
	Id [id] Did [opt dotID] FirstAnnot [annotation_item] Rest [repeat annotation_item] Unused [opt unused_annotation]
    deconstruct FirstAnnot
    	'@ LL MinOffset _ [number] _ [list number]
    by
	Id Did Rest Unused
		    [addIfUnused FirstAnnot]
		    [addIfNoUnused FirstAnnot]
end rule

function addIfNoUnused FirstAnnot [annotation_item]
    replace [opt unused_annotation]
        % empty
    by
    	'! FirstAnnot
end function

function  addIfUnused FirstAnnot [annotation_item]
    replace [opt unused_annotation]
    	'! List [repeat annotation_item]
    by
    	'! List [. FirstAnnot]
end function
...
function addCase MinOffset [number] aTypeRef [type_reference]
    ...
    deconstruct aTypeRef
    	Name [id] Annots [annotation] _ [opt unused_annotation]
...
function addIfValueThere Offset [number] Name [type_reference] aValue [number]
    replace * [switch_case]
        aValue '@ Refs [repeat type_reference]
...
% unused were ambiguous in the current context,
% once partitioned, they might not be ambiguous
% any more.
function moveUnusedBack
    replace [type_reference]
	Id [id] Did [opt dotID] UsableLL [repeat annotation_item] '! Unused [repeat annotation_item]
    by
	Id Did  Unused [. UsableLL]
end function

rule sortCasesByValue
    replace [repeat switch_case]
        N1 [number] '@ List1 [repeat type_reference]
        N2 [number] '@ List2 [repeat type_reference]
...
rule optimizeSubcases Short[id]
    skipping [switch_case]
    replace $ [repeat switch_case]
       Value [number] '@ Action [refs_or_sub_block]
       Rest [repeat switch_case]
    by
       Value '@ Action [buildNestedBlock Short]
       Rest
end rule
