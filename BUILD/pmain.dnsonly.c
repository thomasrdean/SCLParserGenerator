...
int main(int argc, char * argv[]){
	...
	if(ether_type == ETHER_TYPE_ARP) {
		...
		//if (traceFile) fprintf(traceFileParser,"%d/%x/%d/%d/",pdu_arp.hwtype,pdu_arp.protocoltype,pdu_arp.hwsize,pdu_arp.protocolsize);
		//if (traceFile) fprintf(traceFileParser,"%d/%llx/%x/%llx/%x",pdu_arp.opcode, pdu_arp.sendermac,
		//                 pdu_arp.senderip, pdu_arp.targetmac, pdu_arp.targetip);
		// check that parsed result is the same as what went in.
		...
		if (traceFile) fprintf(traceFileParser,"\n");
	    } else { 
		//fprintf(stderr, "Couldn't Parse packet # %d\n", count+1);
		if (traceFile) fprintf(traceFileParser,"\t\t\tFAILED\n");
		++pduFailed;
	    }
	    freePDU_ARP(&pdu_arp);
	    free(thePDU);
		...
		if (parsedPDU == true){
			...
			switch (pdu_igmp.type){
			   case V2Leave_IGMP_VAL:
			       fprintf(traceFileParser, "V2Leave: ");
			       break;
			   case V2Report_IGMP_VAL:
			       fprintf(traceFileParser, "V2Report: ");
			       break;
			   case Query_IGMP_VAL:
			       fprintf(traceFileParser, "Query: ");
			       break;
			   case V3Report_IGMP_VAL:
			       ...
			       break;
			   default:
			       fprintf(traceFileParser, "****ERROR unknown IGMP tag");
			}
		    }
		    if (traceFile) fprintf(traceFileParser,"\n");
		} else { 
		...
		if(traceFileParser) {
		    ...
		    if(val==1381257299) fprintf(traceFileParser, " RTPS");
		    if(val==1381257304) fprintf(traceFileParser, " RTPX");
		    if(((thePDU->data[thePDU->curPos] & 56) == 32) && header->dstPort == 123) fprintf(traceFileParser, " NTP");
		    if((header->srcPort == 53) || (header->dstPort)) fprintf(traceFileParser, " DNS");
		...
		// just DNS for now.
		if((ntohs(up_hdr->uh_sport) == 53) || (ntohs(up_hdr->uh_dport) == 53)){
		    PDU_DNS pdu_dns;
		    if(parsePDU_DNS(&pdu_dns, thePDU, progname, endianness)) {
			if (traceFileParser) fprintf(traceFileParser,"\tSUCCESS");
			SerializeBuffer * buff;
			buff = serializePDU_DNS (NULL, &pdu_dns, argv[1], endianness);
			unsigned long len;
			unsigned char * sdata = combineBuffers(buff,&len);
...
static void print_usage() {
        ...
        fprintf(stderr, "-d | --debugLevel <1> | <2> \n");
...
static void handle_options(int argc, char * argv[]){
	...
	switch(option) {
	    ...
	    case 'd' :{
	        int d = atoi(optarg);
		if(!((d == 1) ||(d == 2))) {fprintf(stderr, "debug level is 1 or 2. Value entered: %s\n", optarg); exit(1); }
		debugLevel = d;
		}
		break;
	    case 'a' :
