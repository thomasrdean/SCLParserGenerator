...
#include "R11_Definitions.h"
#include "R11_Serialize.h"
#include "R11_Print.h"
...
int main() {
    ...
    printf("**********\n");
    PDU_R11 pdu_r11;
    int parsed = parsePDU_R11(&pdu_r11, &thePDU, "R11", endianness);
    if (parsed){
	if (pdu_r11.a != 2) printf("field a is not 2, it is %u\n", pdu_r11.a);
	if (pdu_r11.bcount != 2)printf("count of b is not 2, it is %lu\n", pdu_r11.bcount);
	if (pdu_r11.blength != 6)printf("length of b is not 6, it is %lu\n", pdu_r11.blength);
	if (pdu_r11.b[0].a != 0xA1) printf("b[0].a is not 0xA1, it is %x\n", pdu_r11.b[0].a);
	if (pdu_r11.b[0].b != 0xA2A3) printf("b[0].a is not 0xA2A3, it is %x\n", pdu_r11.b[0].b);
	if (pdu_r11.b[1].a != 0xA4) printf("b[1].a is not 0xA4, it is %x\n", pdu_r11.b[1].a);
	if (pdu_r11.b[1].b != 0xA5A6) printf("b[1].a is not 0xA5A6, it is %x\n", pdu_r11.b[1].b);
	if (pdu_r11.c != 0xA7A8) printf("field c is not 0xA7A8, it is %x\n", pdu_r11.c);
    } else {
	fprintf(stderr,"R11 failed to parse\n");
    }
    printf("**********\n");
    printPDU_R11(stdout,&pdu_r11,0,-1);
    printf("**********\n");

    SerializeBuffer * buff;
    buff = serializePDU_R11 (NULL, &pdu_r11, "R11", endianness);
    printf("**********\n");
    unsigned long len;
